...
void update_RS485_registers_inverter() {
  ...
  if (charge_current_dA > datalayer.battery.status.max_charge_current_dA) {
    charge_current_dA =
        datalayer.battery.status.max_charge_current_dA;  //Cap the value to the max allowed Amp. Some inverters cannot handle large values.
  }

  if (discharge_current_dA > datalayer.battery.status.max_discharge_current_dA) {
    discharge_current_dA =
        datalayer.battery.status.max_discharge_current_dA;  //Cap the value to the max allowed Amp. Some inverters cannot handle large values.
  }
