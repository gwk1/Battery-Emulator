...
void update_values_battery() { /* This function puts fake values onto the parameters sent towards the inverter */
  ...
  {
    datalayer_extended.bmwphev.balancing_allowed=true;
  }
  else if( (balancetime+BALANCE_SEQ_TIME) < currentMillis )
  {
    datalayer_extended.bmwphev.balancing_allowed=false;
  }

  if ((MinCell_mV <= MaxCell_mV - 10) && MaxCell_mV > 3900 && datalayer_extended.bmwphev.balancing_allowed ) {
  ...
  if (MinCell_mV < 3150) {
    datalayer.battery.status.max_discharge_power_W  = datalayer.battery.status.max_discharge_power_W  - map(MaxCell_mV, 3090, 3150, datalayer.battery.status.max_discharge_power_W, 0);
  }
...
void send_can_battery() {
  ...
  if (currentMillis - previousMillis5s >= INTERVAL_10_MS) {
    ...
    if(!datalayer_extended.bmwphev.balancing_active) {
      ...
      TxFrame.data.u8[0] = lowByte(datalayer_extended.bmwphev.balance_target_mV);
      TxFrame.data.u8[1] = highByte(datalayer_extended.bmwphev.balance_target_mV);
    }
